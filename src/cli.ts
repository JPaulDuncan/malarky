...
/**
 * malarky CLI
 * Command-line interface for the malarky text generator
 */
...
const MAIN_HELP = `
malarky - Generate syntactically plausible English text
...
Examples:
  ...
  malarky paragraph --sentences 5 --lexicon ./corp.json
  malarky text --paragraphs 3 --archetype corporate
  malarky sentence --transform pigLatin
  malarky sentence --transform leet,uwu --json
  malarky validate ./my-lexicon.json
  malarky list transforms
`.trimStart();

const SENTENCE_HELP = `
malarky sentence - Generate one or more sentences
...
Examples:
  malarky sentence
  malarky sentence --seed 42 --type question
  malarky sentence --count 5 --lexicon ./corp.json --archetype corporate
  malarky sentence --transform pigLatin --seed 42
  malarky sentence --min-words 10 --max-words 20
`.trimStart();

const PARAGRAPH_HELP = `
malarky paragraph - Generate one or more paragraphs

Usage:
  malarky paragraph [options]
...
Examples:
  malarky paragraph --sentences 5 --seed 42
  malarky paragraph --count 3 --lexicon ./corp.json
  malarky paragraph --transform pirate
`.trimStart();

const TEXT_HELP = `
malarky text - Generate a text block (multiple paragraphs)
...
Examples:
  malarky text --paragraphs 3 --seed 42
  malarky text --lexicon ./corp.json --archetype corporate
  malarky text --transform bizJargon
`.trimStart();

const VALIDATE_HELP = `
malarky validate - Validate a lexicon JSON file
...
const LIST_HELP = `
malarky list - List available transforms or sentence types

Usage:
  malarky list <category>
 ...
 */
function parseTransformOverride(
  values: Record<string, unknown>,
): Partial<OutputTransformsConfig> | undefined {
...
function createGenerator(values: Record<string, unknown>): TextGenerator {
  ...
  if (values['lexicon']) {
    ...
    try {
      ...
      die(
        `Loading lexicon: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
...
function handleSentence(): void {
  ...
  if (
    values['type'] &&
    !VALID_SENTENCE_TYPES.includes(values['type'] as SentenceType)
  ) {
    die(
      `Invalid sentence type "${String(values['type'])}". Valid types: ${VALID_SENTENCE_TYPES.join(', ')}`,
    );
  }
...
function handleValidate(): void {
  ...
  if (!filePath) {
    die('No file path provided.\nUsage: malarky validate <file.json> [--json]');
  }
...
function handleList(): void {
  ...
  if (!category) {
    die('No category specified.\nUsage: malarky list <transforms|types>');
  }
  ...
  switch (category) {
    case 'transforms': {
      ...
      if (values['json']) {
        ...
        for (const t of transforms) {
          const order =
            t.capabilities.preferredOrder !== undefined
    ...
    case 'types': {
      if (values['json']) {
        process.stdout.write(
          JSON.stringify(VALID_SENTENCE_TYPES, null, 2) + '\n',
        );
      } else {
