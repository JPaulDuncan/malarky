...
function makeInput(
  ...
  return {
    ...
    protection: protect
      ? { keepAcronyms: true, keepNumbers: true, minWordLength: 2 }
      : {},
    traceEnabled: false,
  };
}
...
describe('Ubbi Dubbi Transform', () => {
  ...
  it('should handle beforeEachVowel mode', () => {
    const result = applyTransform(ubbiDubbiTransform, 'audio', {
      mode: 'beforeEachVowel',
    });
  ...
  it('should validate params', () => {
    ...
    expect(ubbiDubbiTransform.validateParams({ mode: 'invalid' }).valid).toBe(
      false,
    );
  });
...
describe('Leetspeak Transform', () => {
  ...
  it('should preserve punctuation', () => {
    const result = applyTransform(leetTransform, 'Hello, world!', {
      intensity: 1.0,
    });
    expect(result).toContain(',');
    expect(result).toContain('!');
  ...
  it('should be deterministic with same seed', () => {
    const r1 = applyTransform(
      leetTransform,
      'hello world',
      { intensity: 0.5 },
      100,
    );
    const r2 = applyTransform(
      leetTransform,
      'hello world',
      { intensity: 0.5 },
      100,
...
describe('Pirate Transform', () => {
  it('should substitute common words', () => {
    const result = applyTransform(pirateTransform, 'You are my friend', {
      interjectionRate: 0,
    });
  ...
  it('should preserve case in substitutions', () => {
    const result = applyTransform(pirateTransform, 'My friend', {
      interjectionRate: 0,
    });
    expect(result).toContain('Me');
  });
  ...
  it('should validate params', () => {
    ...
    expect(pirateTransform.validateParams({ interjectionRate: 2 }).valid).toBe(
      false,
    );
  });
  ...
  it('should leave unmatched words unchanged', () => {
    const result = applyTransform(pirateTransform, 'computer science', {
      interjectionRate: 0,
    });
...
describe('Redaction Transform', () => {
  ...
  it('should validate params', () => {
    ...
    expect(redactTransform.validateParams({ style: 'invalid' }).valid).toBe(
      false,
    );
  });

  it('should preserve punctuation', () => {
    const result = applyTransform(redactTransform, 'Hello, world!', {
      rate: 1.0,
      targets: ['any'],
    });
    expect(result).toContain(',');
...
describe('Emoji-speak Transform', () => {
  ...
  it('should not replace unknown words', () => {
    const result = applyTransform(emojiTransform, 'malarky', { rate: 1.0 });
    expect(result).toBe('malarky');
  });

  it('should preserve word alongside emoji when configured', () => {
    const result = applyTransform(emojiTransform, 'fire', {
      rate: 1.0,
      preserveWordAlongsideEmoji: true,
    });
  ...
  it('should preserve punctuation', () => {
    const result = applyTransform(emojiTransform, 'Hello, fire!', {
      rate: 1.0,
    });
    expect(result).toContain(',');
    expect(result).toContain('!');
  ...
  it('should handle custom map', () => {
    const result = applyTransform(emojiTransform, 'test', {
      rate: 1.0,
      map: { test: ['\u2705'] },
    });
    expect(result).toContain('\u2705');
...
describe('Mocking Case Transform', () => {
  ...
  it('should validate params', () => {
    ...
    expect(mockCaseTransform.validateParams({ mode: 'invalid' }).valid).toBe(
      false,
    );
    expect(mockCaseTransform.validateParams({ randomRate: 2 }).valid).toBe(
      false,
    );
  });
...
describe('Word Reverse Transform', () => {
  ...
  it('should validate params', () => {
    ...
    expect(
      reverseWordsTransform.validateParams({ mode: 'invalid' }).valid,
    ).toBe(false);
  });
...
describe('Corporate Jargon Transform', () => {
  ...
  it('should preserve punctuation', () => {
    const result = applyTransform(bizJargonTransform, 'Use this, help them!', {
      rate: 1.0,
    });
    expect(result).toContain(',');
    expect(result).toContain('!');
  ...
  it('should leave unmatched words unchanged', () => {
    const result = applyTransform(bizJargonTransform, 'malarky', { rate: 1.0 });
    expect(result).toBe('malarky');
  });

  it('should preserve whitespace', () => {
    const result = applyTransform(bizJargonTransform, 'use help', {
      rate: 1.0,
    });
    expect(result).toContain(' ');
  });
