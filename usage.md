...
Comprehensive reference for the Malarky text generation library and CLI.
...
Malarky generates six sentence structures. You can request a specific type or let the generator pick one based on weighted probabilities.
...
const businessText = generator.paragraph({
  hints: ['domain:business', 'register:formal'],
});
```
...
malarky sentence --count 10 --min-words 8 --max-words 15
...
malarky paragraph --transform pirate --json
```
...
malarky sentence --transform leet,uwu --seed 42

# Multiple transforms (repeated flags)
malarky sentence -x pirate -x mockCase --seed 42
```
...
```typescript
import { TransformRegistry, createDefaultRegistry } from 'malarky';
import type {
  IOutputTransform,
  TransformInput,
  TransformOutput,
...
malarky sentence --seed 42 --trace

# Capture trace to a file
malarky sentence --seed 42 --trace 2> trace.json
```
...
{
  "outputTransforms": {
    "defaults": [{ "id": "bizJargon" }]
  },
  "archetypes": {
    "pirate-corp": {
      "tags": ["domain:business"],
      "outputTransforms": {
        "pipeline": [{ "id": "pirate" }, { "id": "bizJargon" }]
      }
    }
  }
}
...
```typescript
import { validateLexicon } from 'malarky';
...
```typescript
import { useAn, getIndefiniteArticle, withIndefiniteArticle } from 'malarky';
...
```typescript
import { pluralize, singularize, isPlural } from 'malarky';
...
```bash
malarky paragraph --lexicon examples/lexicons/corporate-min.json --archetype corporate --seed 2024
```
