...
Malarky supports two word provider adapters. The built-in `SimpleFakerAdapter` works with zero dependencies. For more word variety, use `FakerJsAdapter` with the popular `@faker-js/faker` library.

## SimpleFakerAdapter

The default adapter. Ships with Malarky and provides a fixed vocabulary of common English words covering all parts of speech.
